...
if($query->rowCount() > 0)
{
 foreach ($results as $result) {
	...
	/* Find allowed leaves */
    $leaves = mysqli_query($conn, "SELECT leavetype_id, allowed_leaves, inner_name FROM tblallowedleaves, tblleavetype WHERE tblleavetype.id = tblallowedleaves.leavetype_id AND employee_id = {$_SESSION['eid']}");
   
    if( mysqli_num_rows($leaves) > 0 ){
        while( $row = mysqli_fetch_assoc($leaves) ){
            $typeId     = $row['leavetype_id'];
            $innerName  = $row['inner_name'];
            $_SESSION['allowedleaves'][$innerName] = [
                                                        'id'=>$typeId,
                                                        'allowed'=>$row['allowed_leaves']
                                                     ];
        }
    }
  } 
                ...
                <nav class="gray darken-1">
                    <div class="nav-wrapper row">
                        ...
                        <div class="header-title col s5">      
                            <img src="assets/images/dav-logo.png" width="" height="50">
                        </div>
                        <div class="header-title col s5 hide-on-med-and-down">      
                            <span class="chapter-title">Online Leave Portal</span>
                        </div>
                    </div>
                </nav>
            </header>
           ...
           /*
            <aside id="slide-out" class="side-nav white fixed">
                ...
                <ul class="sidebar-menu collapsible collapsible-accordion" data-collapsible="accordion" style="">
                    ...
                    <li class="no-padding"><a href="#"><i class="material-icons">account_box</i>Change Password</a></li>
